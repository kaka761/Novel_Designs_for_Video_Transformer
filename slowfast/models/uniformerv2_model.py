...
### Bio-inspired MinBlock ###
class Min(nn.Module):
    def __init__(self, d_model):
        super().__init__() 
        self.conv1 = nn.Conv3d(d_model, d_model, kernel_size=1, stride=1, padding=0) #  torch.Size([12, 256, 4, 14, 14])
        self.conv2 = nn.Conv3d(d_model, d_model, kernel_size=1, stride=1, padding=0) #  torch.Size([12, 256, 4, 14, 14])
        self.conv3 = nn.Conv3d(d_model, d_model, kernel_size=1, stride=1, padding=0) # torch.Size([12, 256, 4, 14, 14])
    ...
    def forward(self, x):
        ...
        x2 = self.conv2(x)
        x3 = self.conv3(x)
        z1 = torch.minimum(x1, x2)
        z2 = torch.minimum(x2, x3)
        z3 = torch.minimum(x1, x3)
        z = z1 + z2 + z3
        return z
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        if temporal_downsample:
            ...
            # 3 shapes #
            self.conv1 = nn.Conv3d(3, width//3, (1, patch_size, patch_size), (8, patch_size, patch_size), (0, 0, 0), bias=False)
            self.conv2 = nn.Conv3d(3, width//3, (4, patch_size, patch_size), (8, patch_size, patch_size), (0, 0, 0), bias=False)
            self.conv3 = nn.Conv3d(3, width//3, (8, patch_size, patch_size), (8, patch_size, patch_size), (2, 0, 0), bias=False)
            t_size = t_size // 8
            ###-----------------------------###       

        self.conv4 = Min(width) # MinBlock
...
### For using pretrained weights for Variable Tubes Tokenization ###
def load_state_dict_tubes(model, state_dict):
    state_dict_3d = model.state_dict()
    time_dim = state_dict_3d['conv1.weight'].shape[2]
    tem = inflate_weight(state_dict['conv1.weight'], time_dim)
    #state_dict['conv1.weight'] = tem[0:256,:,:,:]
    #state_dict['conv2.weight'] = tem[256:512,:,:,:]
    state_dict['conv3.weight'] = tem[512:768,:,:,:]
    state_dict.move_to_end('conv3.weight', last = False)
    #state_dict.move_to_end('conv2.weight', last = False)
    #state_dict.move_to_end('conv1.weight', last = False)
    state_dict.move_to_end('positional_embedding', last = False)
    state_dict.move_to_end('class_embedding', last = False)
    state_dict.pop('conv1.weight') # delete key ''conv1.weight''
    for v in state_dict:
        print(v, state_dict[v].shape)
    model.load_state_dict(state_dict, strict=False) # load pretrained weights
