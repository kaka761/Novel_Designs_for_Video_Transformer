...
def decode(
    ...
    if backend in ["pyav", "torchvision"]:
        ...
        if sparse:
            ...
            ###-------------###
                ### for RGBt ###
                seq = get_seq_frames(len(frames), num_frames*3, clip_idx, num_clips)
                seq3 = np.reshape(seq, (-1, 3))
                seq0 = seq3[:, 0]
                seq1 = seq3[:, 1]
                seq2 = seq3[:, 2]
            frames0 = frames.get_batch(seq0)
            frames1 = frames.get_batch(seq1)
            frames2 = frames.get_batch(seq2)
            ###--------------###
        else:
